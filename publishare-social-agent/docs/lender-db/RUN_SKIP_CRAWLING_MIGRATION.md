# Run Skip Crawling Migration

## Purpose

Adds a `skip_crawling` flag to the `lenders` table to mark invalid/category lenders that should be skipped during automated crawling.

## Migration File

`supabase/migrations/20251202000002_add_skip_crawling_flag.sql`

## Steps

1. **Open Supabase SQL Editor**
   - Go to: https://supabase.com/dashboard/project/vpysqshhafthuxvokwqj/sql/new
...
AND column_name = 'skip_crawling';

-- Check marked lenders
...
- ✅ Column `skip_crawling` added to `lenders` table
- ✅ Index created for performance
- ✅ 8 lenders marked to skip crawling:
  - 5 invalid lenders (high confidence)
  - 3 suspicious lenders (medium confidence)

## After Migration

1. Run the mark script to verify:
   ```bash
   npm run mark-invalid:dry-run
   ```

2. The crawler will automatically skip these lenders
